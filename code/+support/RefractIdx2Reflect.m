classdef RefractIdx2Reflect < matlab.System % codegen
    % REFRACTIDX2REFLECT: Map from refractive index to reflection
    %
            ...
            elseif strcmp(obj.OutputMode,'Jacobian')
                ...
                elseif strcmp(obj.PhiMode,'Signed-Quadratic')
                    ...
                    for iCol = 1:nCols
                        mask = zeros(size(arrayU));
                        mask(iCol) = 1;
                        vecD = 2*beta2*obj.dltFcn.step(mask.*abs(arrayDltU)); % Flip sign for adjoint
                        arrayD(:,iCol) = vecD(:);
                    end
                ...
                elseif strcmp(obj.PhiMode,'Reflection')
                    ...
                    for iCol = 1:nCols
                        mask = zeros(size(arrayU));
                        mask(iCol) = 1;
                        vecD = 2*( obj.dltFcn.step(mask.*arrayAddU.*arrayT)... % Flip sign for adjoint
                            + obj.adtFcn.step(mask.*arrayDltU.*arrayT) );      % Unchanged for adjoint
                        arrayD(:,iCol) = vecD(:);
                    end
